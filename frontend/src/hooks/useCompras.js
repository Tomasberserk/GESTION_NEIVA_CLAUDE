...
export function useCompras() {
  ...
  const cargar = useCallback(async (filtros = {}) => {
    ...
    try {
      ...
      const res = await authService.fetchAuth(`${BASE}/compras?${params}`)
  ...
  const registrar = useCallback(async (payload) => {
    const res = await authService.fetchAuth(`${BASE}/compras`, {
      method: 'POST',
  ...
  const anular = useCallback(async (id) => {
    const res = await authService.fetchAuth(`${BASE}/compras/${id}`, { method: 'DELETE' })
    if (!res.ok) {
      const err = await res.json()
