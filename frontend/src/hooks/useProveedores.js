...
export function useProveedores() {
  ...
  const cargar = useCallback(async (q = '') => {
    ...
    try {
      ...
      if (q) params.set('q', q)
      const res = await authService.fetchAuth(`${BASE}/proveedores?${params}`)
  ...
  const crear = useCallback(async (payload) => {
    const res = await authService.fetchAuth(`${BASE}/proveedores`, {
      method: 'POST',
  ...
  const actualizar = useCallback(async (id, payload) => {
    const res = await authService.fetchAuth(`${BASE}/proveedores/${id}`, {
      method: 'PUT',
      body: JSON.stringify(payload),
    })
    if (!res.ok) {
  ...
  const eliminar = useCallback(async (id) => {
    const res = await authService.fetchAuth(`${BASE}/proveedores/${id}`, { method: 'DELETE' })
    if (!res.ok) {
      const err = await res.json()
