...
export function useCuentasPorPagar() {
  ...
  const cargar = useCallback(async (filtros = {}) => {
    ...
    try {
      ...
      const res = await authService.fetchAuth(`/api/cuentas-por-pagar?${params}`)
  ...
  const registrarAbono = useCallback(async (cxpId, payload) => {
    const res = await authService.fetchAuth(`/api/cuentas-por-pagar/${cxpId}/abonos`, {
      method: 'POST',
  ...
  const reversarAbono = useCallback(async (abonoId) => {
    const res = await authService.fetchAuth(`/api/cuentas-por-pagar/abonos/${abonoId}`, {
      method: 'DELETE',
    })
    if (!res.ok) {
      const err = await res.json()
